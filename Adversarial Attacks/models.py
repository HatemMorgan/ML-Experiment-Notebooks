...
class SmoothClassifier(nn.Module):
    ...
    def certify(self, inputs: torch.Tensor, n0: int, num_samples: int, alpha: float, batch_size: int) -> Tuple[int,
        ...
        ##########################################################
        # YOUR CODE HERE
        # compute most likely class c*
        # class_counts = self._sample_noise_predictions(inputs, n0, batch_size) # draw samples of f(x+ epsilon)
        # top_class = torch.argmax(class_counts).item() # get the top class
        ...
        # compute lower bound of p_x*
        class_counts = self._sample_noise_predictions(inputs, num_samples, batch_size) # draw another samples of f(x+ epsilon)
        top_class_counts = class_counts[top_class].item()
        p_A_lower_bound = lower_confidence_bound(top_class_counts, num_samples, alpha)
        ##########################################################
        ...
        if p_A_lower_bound < 0.5:
            ...
            radius = self.sigma * norm.ppf(p_A_lower_bound) # ppf(prob) is inverse CDF of a standard Gaussian
            ##########################################################
    ...
    def predict(self, inputs: torch.tensor, num_samples: int, alpha: float, batch_size: int) -> int:
        ...
        """
        self.base_classifier.eval() # setting base model in evaluation (test) mode
        class_counts = self._sample_noise_predictions(inputs, num_samples, batch_size).cpu()
        ##########################################################
        # YOUR CODE HERE
        _, indices = torch.sort(class_counts)
        
        top_class = indices[-1]
        count1 = class_counts[top_class] # number of occurences of top class
        count2 = class_counts[indices[-2]] # number of occurences of second top class
        if(binom_test(count1, count1 + count2, p=0.5) > alpha): # confidence level = 1 - alpha
...
pytorch nn.module eval        batch_size: int
            The batch size to use during the prediction, i.e. how many noise samples to classify in parallel.
        ...
        with torch.no_grad():
            # get an array of containing class numbers (e.g. [1,2,3])
            ...
            for it in range(ceil(num_samples / batch_size)):
                ...
                # repeat inputs this_batch_size time, shape[this_batch_size, C, N, N]
                input_batch = inputs.repeat(this_batch_size, 1, 1, 1)
                logits_pert = self.forward(input_batch) # smooth classifier forward pass, shape [this_batch_size, B]
                labels_pert = torch.argmax(logits_pert.data, 1) # get index (winning class label) of maximum logit
                
                # count occurences .of each class
                class_pert, class_pert_counts = labels_pert.unique(sorted=False,return_counts=True)
                
                # update class_counts
                for i, class_idx in enumerate(class_pert):
                  class_counts[class_idx] += class_pert_counts[i]
                ##########################################################
        return class_counts
